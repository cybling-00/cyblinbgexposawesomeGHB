      ...
      <v-row no-gutters>
        ...
        <v-col cols="2">
          <v-row no-gutters class="pa-1 pt-2 pl-0">
              <v-btn block class="pa-0 m-1" color="golden" @click="check_if_is_deposit_return_item(i)" dark v-for="i in allItems" v-if="i.custom_is_deposit_return === 1">{{ i.item_code }}</v-btn>
          </v-row>
        </v-col>
        <v-col cols="5">
          <v-row no-gutters class="pa-1 pt-2 pl-0">
...
export default {
  ...
  methods: {
    ...
    async check_make_return() {
      if (!this.return_doc.name){
        this.invoice_doc = {}
        this.return_doc = {}
      }
    ...
    async add_is_deposit_return_item(item, item_already_set) {
      if(!item_already_set){
        this.items = [];
        item.qty = -1
      }
      this.add_item(item)
      this.check_make_return()
    },
    async check_if_is_deposit_return_item(item) {
      let item_already_set = false
      for (let row of this.items || []){
        if(row.item_code == item.item_code){
          item_already_set = true
        }
      }
      if(!item_already_set && this.items.length > 0){
        frappe.confirm('Are you sure you want to remove these selected items and Create a new Return Invoice?',
          () => {
              // action to perform if Yes is selected
              this.add_is_deposit_return_item(item, item_already_set)
          }, () => {
