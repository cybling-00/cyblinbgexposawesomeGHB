...
export default {
  ...
  data: () => ({
    ...
    mpesa_modes: [],
    popup_amount: "",
    popup_pre_selected_val: false,
    jquery_func_rendered: false,
  }),
  ...
  methods: {
    ...
    submit_invoice(print) {
      ...
      if ( this.invoice_doc.is_return === 1  &&
        totalPayedAmount != (this.invoice_doc.rounded_total || this.invoice_doc.grand_total || this.invoice_doc.total)) {
        evntBus.$emit("show_mesage", {
          text: `Return Amount is not Correct.`,
          color: "error",
        });
        frappe.utils.play_sound("error");
        return;
    ...
    get_enter_amount_dialog_html() {
      let currencySymbol = this.currencySymbol(this.invoice_doc.currency)
      return `<div class="popup_ui_container">
        <p>
            <button class="popup_Button_click popup_Button" value-to-update='1' >1</button>
            <button class="popup_Button_click popup_Button" value-to-update='2' >2</button>
            <button class="popup_Button_click popup_Button" value-to-update='3' >3</button>
            <button class="popup_Button_click popup_Curreny-Button popup_10-Curreny-Button" value-to-update='10' value-to-reset=true value-to-pre_val=true >${currencySymbol} 10</button>
        </p>
        <p>
            <button class="popup_Button_click popup_Button" value-to-update='4' >4</button>
            <button class="popup_Button_click popup_Button" value-to-update='5' >5</button>
            <button class="popup_Button_click popup_Button" value-to-update='6' >6</button>
            <button class="popup_Button_click popup_Curreny-Button popup_20-Curreny-Button" value-to-update='20' value-to-reset=true value-to-pre_val=true >${currencySymbol} 20</button>
        </p>
        <p>
            <button class="popup_Button_click popup_Button" value-to-update='7' ">7</button>
            <button class="popup_Button_click popup_Button" value-to-update='8' ">8</button>
            <button class="popup_Button_click popup_Button" value-to-update='9' ">9</button>
            <button class="popup_Button_click popup_Curreny-Button popup_50-Curreny-Button" value-to-update='50' value-to-reset=true value-to-pre_val=true >${currencySymbol} 50</button>
        </p>
        <p>
            <button class="popup_Button_click popup_Button" value-to-update='.' >.</button>
            <button class="popup_Button_click popup_Button" value-to-update='0' >0</button>
            <button class="popup_Button_click popup_Button popup_delete-Button" value-to-update='del' value-to-reset=true >Delete</button>
            <button class="popup_Button_click popup_Curreny-Button popup_100-Curreny-Button" value-to-update='100' value-to-reset=true value-to-pre_val=true >${currencySymbol} 100</button>
        </p>
    </div>`
    },
    UpdateCurrency(value, reset = false, pre_val = false) {
      if (reset === 'true') {
        this.popup_amount = "";
      }
      if (value === 'del') {
        ...
        if (!this.popup_amount.includes(value)) {
          this.popup_amount += String(value);
        }
      } else {
        if (this.popup_amount == "0" || this.pre_selected_val === 'true') {
          this.popup_amount = ""
        }
        this.popup_amount += String(value);
      }
      this.pre_selected_val = pre_val
    ...
    DisplayCurrency() {
      ...
      if(dialog){
        ...
        let to_pay = flt(dialog.get_value("to_pay"))
        dialog.set_value("amount", this.popup_amount)
        let change = (flt(this.popup_amount) - to_pay)
        if(change> 0){
          dialog.set_value("change", change)
        }
      }
    },
    make_enter_amount_dialog(idx) {
      let vm = this
      vm.popup_amount =  ""
      vm.popup_pre_selected_val =  false
      let enter_amount_dialog_html = vm.get_enter_amount_dialog_html()
          ...
          fields: [
              {
                ...
                fieldname: 'to_pay',
                fieldtype: 'Currency',
                default: Math.abs(this.invoice_doc.rounded_total) || Math.abs(this.invoice_doc.grand_total),
                read_only: 1,
              },
              {
              ...
              },
              {
                  ...
                  fieldname: 'html',
                  fieldtype: 'HTML',
                  options: enter_amount_dialog_html
              },
          ],
          size: 'large',
          primary_action_label: 'Submit',
          primary_action(values) {
              vm.set_amount_from_pop_up(idx, values)
              dialog.hide();
          }
      });
      if(!vm.jquery_func_rendered){
        vm.jquery_func_rendered = true;
        $( "body" ).delegate( ".popup_Button_click", "click", function() {
          let value = $(this).attr("value-to-update")
          let reset = $(this).attr("value-to-reset")
          let pre_val = $(this).attr("value-to-pre_val")
          vm.UpdateCurrency(value, reset, pre_val)
          console.log(value)
        });
      }
      return dialog
    ...
    set_full_amount(idx) {
      this.popup_amount =  ""
      this.popup_pre_selected_val =  false
      this.invoice_doc.payments.forEach((payment) => {
        if (payment.idx == idx){
          if (payment.type == "Cash"){
            let dialog = this.make_enter_amount_dialog(idx);
            dialog.show();
          }else{
    ...
    set_amount_from_pop_up(idx, values) {
      this.invoice_doc.payments.forEach((payment) => {
        if (payment.idx == idx){
            if (this.invoice_doc.is_return==1){
              payment.amount = -cint(values.amount);
            }else{
              payment.amount = cint(values.amount);
            }
        }else{
...
.popup_Button {
    ...
    border: none;
    box-shadow: 0.01px 3px 3px 3px rgba(0, 0, 0, 0.1);
    font-family: Arial, Helvetica, sans-serif;
    font-size: 25px;
}

.popup_Curreny-Button {
    background-color: rgb(255, 255, 255);
    margin-left: 10px;
    padding-left: 10px;
    padding-right: 10px;
